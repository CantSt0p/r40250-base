			...
			if (Cell.IsBeltInventoryPosition())
			{
				...
				if (m_pointsInstant.bItemGrid[bCell]) 
				{
					if (m_pointsInstant.bItemGrid[bCell] == iExceptionCell) 
					{
						...
						{
							const WORD globalSlot = bCell + (BELT_INVENTORY_SLOT_WIDTH * j);
							const WORD localSlot = globalSlot - BELT_INVENTORY_SLOT_START;

							if (localSlot >= BELT_INVENTORY_SLOT_COUNT ||
								!CBeltInventoryHelper::IsAvailableCell(localSlot, beltItem->GetValue(0)) ||
								(m_pointsInstant.bItemGrid[globalSlot] && m_pointsInstant.bItemGrid[globalSlot] != iExceptionCell)) 
								{
									return false;
								}
						}
